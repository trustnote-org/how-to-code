...
class AdminVideoScreen < Sinatra::Base
    ...
    get '/admin/landingPage/video/edit/:id' do
        unless session[:username]
            halt "Access denied, please <a href='/login'>login</a>."
        end
        ...
        db.execute( "select id,icon,title,text,url from video where id = '#{id}'" ) do |row|
    ...
    post '/admin/landingPage/video/edit/:id' do
        ...
        sql ="update video set `title`='#{title}',`text`='#{text}',`icon`='#{icon}',`url`='#{url}' where id = '#{id}'"
        db.execute( sql )
        redirect '/admin/landingPage/video/all'
    end
